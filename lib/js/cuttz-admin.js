jQuery(document).ready(function(jQuery) {
	...
	if(typeof cuttz !== 'undefined' && cuttz.cuttz_fonts){
		jQuery.each(font_fields,function(key, value){
			jQuery("."+key).chosen();
			jQuery("."+key).on('change', function(evt, params) {
					selected_font = params.selected;
					cuttz_fonts = JSON.parse(cuttz.cuttz_fonts);
					
					options = get_font_variant_html(cuttz_fonts[selected_font].font_weights);
					jQuery("."+value).empty().append(weightoptions)
				});
		});
	}
	
	jQuery('#cuttz_extras_license .gfonts-api-key').mouseover(function() {
		
		if(jQuery('#cuttz_extras_license .gfonts-api-key').val().length != 0) {
			jQuery('#cuttz_extras_license .gfonts-api-key').removeClass('validated').addClass('validating');

			jQuery.ajax({
				method: 'GET',
				cache: false,
				url: 'https://www.googleapis.com/webfonts/v1/webfonts?sort=alpha&key='+jQuery('#cuttz_extras_license .gfonts-api-key').val(),
				dataType: 'json',
				success: function(data){
					jQuery('#cuttz_extras_license .gfonts-api-key').removeClass('validating valid invalid empty').addClass('validated valid');
				},
				error: function(xhr, type, exception) { 
					// if ajax fails display error alert
					//alert("Oops... ajax error response type "+type);
					jQuery('#cuttz_extras_license .gfonts-api-key').removeClass('validating valid invalid empty').addClass('validated invalid');
				}
			});
		}
		else {
			jQuery('#cuttz_extras_license .gfonts-api-key').addClass('empty');
		}
		
	});	
			
	// 'use strict';
	jQuery('.cuttz-color-selector','.gl-section').each(function (i) {
		jQuery(this).after('<div class="cuttz-color-picker" id="picker-' + i + '" style="position:absolute;z-index:9999;right:50px;"></div>');
	...
	if(typeof cuttz !== 'undefined' && cuttz.firstTime){
	...
	jQuery('<input id="toggle-meta-boxes" type="button" class="button button-secondary lexpand" value="Toggle All" />"') 
		...
		.click(function() {
			...
			jQuery('#cuttz_extras_help_box').removeClass('closed');				
			jQuery('#cuttz_design_help_box').removeClass('closed');
	...
	jQuery('.cuttz-lp-layout input:checked').each( function() {
		jQuery('.cuttz-lp-layout input[name="' + jQuery(this).attr('name') + '"]:checked').parent('label').addClass('active');
	});
	
	// Append classes to Cuttz page / post layout style selector markup
	
	jQuery('.cuttz-lp-layout input').click(function() {
		jQuery('.cuttz-lp-layout input[name="' + jQuery(this).attr('name') + '"]').parent('label').removeClass('active');
