...
add_shortcode('button', 'cuttz_cta_button');
 ...
 * @since 1.0
 */
function cuttz_cta_button($atts, $content = null) {
...
add_shortcode('big-button', 'cuttz_big_cta_button');
 ...
 * @since 1.0
 */
function cuttz_big_cta_button($atts, $content = null) {
   extract(shortcode_atts(array(
		'link' => '#',
		'color' => 'cuttz-blue',
		'type' => 'transparent',
...
add_shortcode('white-button', 'cuttz_white_button');
 ...
 * @since 1.0
 */
function cuttz_white_button($atts, $content = null) {
   extract(shortcode_atts(array(
		'link' => '#',
		'hovercolor' => 'cuttz-blue',
		'type' => 'transparent',
		'round' => "no",
		'icon' => '',
		"newwindow" => "no",
...
add_shortcode('cuttz-list', 'cuttz_list_styles');
 ...
 * @since 1.0
 */
function cuttz_list_styles($atts, $content = null) {
   extract(shortcode_atts(array(
		'color' => 'cuttz-blue',
		'style' => '',
		"id" => '',
	...
	$cuttz_list =  '<div'.$id.' class="cuttz-list '.esc_attr($style).' '. esc_attr($color).''.esc_attr($user_class).'"> '.do_shortcode($content). '</div>';
	return $cuttz_list;
}


add_shortcode('collapsible-content', 'cuttz_collapsible_content');
 ...
 * @since 1.0
 */
function cuttz_collapsible_content($atts, $content = null) {
	
	extract(shortcode_atts(array (
		'caption' => __( 'Click here to read more', 'cuttz-framework' ),
		'state' => 'close',
		'id' => '',
		'class' => ''
	), $atts ));
