...
function cuttz_font_weights( $font_id ) {
	if ( $font_id ) {
		$font_specs = cuttz_get_font( $font_id );
...
/**
 * Returns the font family of a given font
 * @param string $value 
 * @return string
 * @since 1.0
 */
function cuttz_fontfamily( $value = 'inherit' ) {
	$font_faces = cuttz_get_fonts();
 ...
 * @since 1.0
 */
function cuttz_get_font_weight( $variant ) {
 ...
 * @since 1.0
 */
function cuttz_get_font_style( $variant ) {
	$style = "normal";
 ...
 * @since 1.0
 */
function cuttz_get_font( $font ) {
	$all_fonts = cuttz_get_super_fonts();
	$font      = $all_fonts[$font];
	return $font;
}
...
/**
 * Builds and returns the complete list of available fonts for use in Cuttz
 * @return string fonts list
 * @since 1.0
 */
function cuttz_get_super_fonts() {

	// Uncomment this to force a refresh
	//delete_transient( 'cuttz_fonts_list' );

	if ( !get_transient( 'cuttz_fonts_list' ) ) { // if the fonts are not already saved in a transient then build the array and save it into a transient with 24hr expiry.
		$json = array();
		...
		set_transient( 'cuttz_fonts_list', $fontslist, 60 * 60 * 24 ); //Expires in 24 hours
		return $fontslist;
	} else {
		return get_transient( 'cuttz_fonts_list' );
	}
