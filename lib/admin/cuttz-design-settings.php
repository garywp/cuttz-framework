...
add_filter( 'genesis_available_sanitizer_filters', 'cuttz_bool_sanitizer' );
 ...
 * @since 1.0
 */
function cuttz_bool_sanitizer( $filters ) {
	$filters['cuttz_bool_string'] = 'cuttz_bool_string';
	return $filters;
}
 ...
 * @since 1.0
 */
function cuttz_bool_string( $new, $old ) {
	if ( !$new ) {
		return "false";
	} else {
		return "true";
...
add_filter( 'genesis_available_sanitizer_filters', 'cuttz_color_sanitizer' );
 ...
 * @since 1.0
 */
function cuttz_color_sanitizer( $filters ) {
	$filters['cuttz_validate_color'] = 'cuttz_validate_color';
	return $filters;
}

add_filter( 'genesis_available_sanitizer_filters', 'cuttz_font_size_sanitizer' );
 ...
 * @since 1.0
 */
function cuttz_font_size_sanitizer( $filters ) {
	$filters['cuttz_font_size'] = 'cuttz_font_size';
	return $filters;
}
 ...
 * @since 1.0
 */
function cuttz_font_size( $new, $old ) {
	return max( 8, $new );
}

add_filter( 'genesis_available_sanitizer_filters', 'cuttz_min_width_sanitizer' );
 ...
 * @since 1.0
 */
function cuttz_min_width_sanitizer( $filters ) {
	$filters['cuttz_min_width'] = 'cuttz_min_width';
	return $filters;
}
 ...
 * @since 1.0
 */
function cuttz_min_width( $new, $old ) {
	return max( 50, $new );
}

/**
 * 
 * Class that registers and outputs a design options page based on the current skin set in Cuttz Admin Settings.
 */
class Cuttz_Skin_Settings extends Genesis_Admin_Boxes {
	private $cuttz_fonts;
	function __construct() {
		$page_id  = cuttz_get_skin_page_id();
		$menu_ops = array(
			'submenu' => array(
				'parent_slug' => __( 'genesis', 'cuttz-framework' ),
				'page_title' => sprintf( __( '&#8220;%s&#8221; Skin Design &mdash; Cuttz Framework', 'cuttz-framework' ), cuttz_get_skin_page_title() ),
				'menu_title' => __( 'Cuttz Skin Design', 'cuttz-framework' )
			)
		);
		$page_ops = array(
			'screen_icon' => 'themes',
			'save_button_text' => sprintf( __( 'Save %s Options', 'cuttz-framework' ), cuttz_get_skin_page_title() ),
			'reset_button_text' => sprintf( __( 'Reset %s To Defaults', 'cuttz-framework' ), cuttz_get_skin_page_title() )
		);

		$settings_field     = cuttz_get_skin_page_id();
		$this->cuttz_fonts = cuttz_get_super_fonts();
		$this->create( $page_id, $menu_ops, $page_ops, $settings_field, cuttz_skin_defaults() );
	...
	function sanitization_filters() {
		...
		genesis_add_option_filter( 'cuttz_min_width', $this->settings_field, array(
				'column-content-1col',
				'column-content-2col',
				'sidebar-one-2col',
				'column-content-3col',
		...
		genesis_add_option_filter( 'cuttz_font_size', $this->settings_field, array(
				'body-font-size',
				'form-font-size',
		...
		genesis_add_option_filter( 'cuttz_validate_color', $this->settings_field, array(
	...
	function metaboxes() {
		...
		add_meta_box( 'cuttz_layout_settings_box', __( 'Layout Widths', 'cuttz-framework' ), array(
			$this,
			'layout_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_layout_styles_box', __( 'Layout Styles', 'cuttz-framework' ), array(
			$this,
			'layout_styles_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_b_fonts_settings_box', __( 'Body Font &amp; Globals', 'cuttz-framework' ), array(
			$this,
			'b_font_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_site_heading_settings_box', __( 'Site Header', 'cuttz-framework' ), array(
			$this,
			'site_header_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_primary_navmenu_settings_box', __( 'Primary Navigation Menu', 'cuttz-framework' ), array(
			$this,
			'primary_navmenu_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_secondary_navmenu_settings_box', __( 'Secondary Navigation Menu', 'cuttz-framework' ), array(
			$this,
			'secondary_navmenu_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_headline_settings_box', __( 'Content Headlines', 'cuttz-framework' ), array(
			$this,
			'headline_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_byline_meta_settings_box', __( 'Byline and Post Meta Data', 'cuttz-framework' ), array(
			$this,
			'byline_meta_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_sidebar_settings_box', __( 'Sidebars', 'cuttz-framework' ), array(
			$this,
			'sidebar_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_footer_widgets_settings_box', __( 'Footer Widgets', 'cuttz-framework' ), array(
			$this,
			'footer_widgets_settings_box'
		), $this->pagehook, 'main' );

		add_meta_box( 'cuttz_footer_settings_box', __( 'Footer', 'cuttz-framework' ), array(
			$this,
			'footer_settings_box'
		), $this->pagehook, 'main' );
		...
		do_action( 'cuttz_skin_after_metaboxes', $this );
	...
	function scripts() {
		...
		wp_localize_script( 'cuttz-admin-js', 'cuttz-framework', array(
			...
			'cuttz_fonts' => json_encode( $this->cuttz_fonts )
		) );
	...
	function styles() {
		...
		wp_enqueue_style( 'cuttz-chosen-style', CHILD_URL . '/lib/css/chosen.css' );
	...
	function hidden_fields( $pagehook ) {
		...
		$ff = genesis_get_option( 'cuttz-web-fonts', $this->settings_field, false );		
		?>
		<input type="hidden" id="<?php echo $this->settings_field; ?>[cuttz-web-fonts]" name="<?php echo $this->settings_field; ?>[cuttz-web-fonts]" value="<?php echo $ff; ?>" />
		<input type="hidden" id="<?php echo $this->settings_field; ?>[skin-version]" name="<?php echo $this->settings_field; ?>[skin-version]" value="<?php echo CHILD_THEME_VERSION; ?>" />
		<?php
	}
...
?>
	...
	function layout_settings_box() {
		  ...
		  <div class="gl-desc">
			<table class="gl-col-table">
				<tr>
					<td colspan="3">
						<h4 class="gl-head"><?php
		_e( 'Genesis Layout Widths', 'cuttz-framework' );
		?></h4>
						<p class="l-desc"><?php
		_e( 'You can use these settings to specify the column widths and padding between the columns across all six Genesis layouts.', 'cuttz-framework' );
		?></p>
					</td>
				</tr>
				<tr>
		...
		echo $this->settings_field;
		?>[col-spacing]"><?php
		_e( 'Padding Between Columns', 'cuttz-framework' );
		?></label></p>
					</td>
					<td class="cuttz-input">
		...
		?>" />&nbsp;px</p>
				...
				<tr>
					<td colspan="3">
						<h4 class="gl-head"><?php
		_e( 'Single Column Layout', 'cuttz-framework' );
		?></h4>
					</td>
				</tr>
				<tr>
		...
		_e( 'Content Column Width', 'cuttz-framework' );
		?></label></p>
					</td>
					<td class="cuttz-input"><p>
		...
		_e( '<strong>Tip: </strong>Make this a total of <span class="ans"></span> to keep the page-width consistent across different layouts.', 'cuttz-framework' );
		?></p></td>
				</tr>
				<tr>
				<td colspan="3"><h4 class="gl-head"><?php
		_e( 'Two Column Layout', 'cuttz-framework' );
		?></h4></td>
			  </tr>
		...
		_e( 'Primary Sidebar Width', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
		...
		?>" />&nbsp;px</p></td>
			  ...
			  <tr>
				<td colspan="3"><h4 class="gl-head"><?php
		_e( 'Three Column Layout', 'cuttz-framework' );
		?></h4>
				</td>
			  </tr>
		...
		_e( 'Secondary Sidebar Width', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
...
?>
		...
		?>"><?php
		_e( 'Primary Font Family', 'cuttz-framework' );
		?></label></p>
					</td>
					<td class="cuttz-input">
					<p>
		...
		foreach ( $this->cuttz_fonts as $font_key => $font ) {
...
?>
		...
		echo $this->settings_field;
		?>[body-font-size]"><?php
		_e( 'Primary Font Size', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="number" min="8" max="200" id="<?php
		echo $this->settings_field;
		...
		_e( 'Primary Font Variant', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'body-font-family', $this->settings_field, false ) );
...
?>
		...
		_e( 'Link Hover Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Form Fields Font Family', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<?php
...
?>
		...
		echo $this->settings_field;
		?>[form-font-size]"><?php
		_e( 'Form-Fields Font Size', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="number" min="8" max="200" id="<?php
		...
		echo $this->settings_field;
		?>[form-text-color]"><?php
		_e( 'Form Fields Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
...
?>
		...
		_e( 'Customize The Look Of Site-Title.', 'cuttz-framework' );
		?></h4>
				</td>
			</tr>
			<tr>
		...
		?>"><?php
		_e( 'Font Family', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<?php
...
?>
		...
		_e( 'Font Variant', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'site-title-font-family', $this->settings_field, false ) );
...
?>
		...
		?>" />
			...
			<table class="gl-col-table">
			 <tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Customize The Look Of Site-Tagline.', 'cuttz-framework' );
		?></h4>
				</td>
			</tr><tr>
				<td class="gl-label"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'site-description-font-family', $this->settings_field, false ) );
...
?>
		...
		_e( 'Post Titles', 'cuttz-framework' );
		?></h4></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
...
<tr>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'headline-font-family', $this->settings_field, false ) );
...
?>
		...
		?>" />
			  ...
			  <tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Headlines Inside The Post Body', 'cuttz-framework' );
		?></h4></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
...
<tr>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'headline-subhead-font-family', $this->settings_field, false ) );
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'nav-menu-font-family', $this->settings_field, false ) );
...
?>
		...
		_e( 'Nav Menu Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Nav Menu Hover Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Current Link Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Parent Nav Link Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Nav Menu Link Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Nav Menu Link Hover Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Current Link Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Parent Nav Link Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		?>" />
			  ...
			  <tr>
				<td class="gl-label"><p>
					<label for="<?php
		echo $this->settings_field;
		?>[nav-menu-border-width]"><?php
		_e( 'Nav Menu Border Width', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input maxlength="1" type="number" id="<?php
		echo $this->settings_field;
		?>[nav-menu-border-width]" name="<?php
		echo $this->settings_field;
		?>[nav-menu-border-width]" value="<?php
		echo genesis_get_option( 'nav-menu-border-width', $this->settings_field, false );
		?>" />
				  </p></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
					<label for="<?php
		echo $this->settings_field;
		?>[nav-menu-border-color]"><?php
		_e( 'Nav Menu Border Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Submenu Width', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'subnav-menu-font-family', $this->settings_field, false ) );
...
?>
		...
		_e( 'Menu Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Menu Hover Text Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Menu Link Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
		...
		_e( 'Menu Link Hover Background Color', 'cuttz-framework' );
		?></label>
				  </p></td>
				<td class="cuttz-input"><p>
					<input type="text" class="color_validate cuttz-color-selector" id="<?php
...
?>
		...
		_e( 'These settings affect the post bylines (entry-meta) of the posts & pages.', 'cuttz-framework' );
		?></p></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'byline-font-family', $this->settings_field, false ) );
...
?>
		<div class="gl-section">
		  <div class="gl-desc">
			...
			<tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Sidebar Widget Headings', 'cuttz-framework' );
		?></h4></td>
			</tr>
			<tr>
				<td class="gl-label"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'sidebar-heading-font-family', $this->settings_field, false ) );
...
?>
		...
		?>" />
			  ...
			  <tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Sidebar Widget Body', 'cuttz-framework' );
		?></h4></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'sidebar-font-family', $this->settings_field, false ) );
		...
		<div class="gl-section">
		  <div class="gl-desc">
			<table class="gl-col-table">
			  <tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Footer Widget Headings', 'cuttz-framework' );
		?></h4></td>
			  </tr>
			  <tr>
				<td class="gl-label"><p>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'footer-widgets-heading-font-family', $this->settings_field, false ) );
...
?>
		...
		?>" />
			  ...
			  <tr>
				<td colspan="2"><h4 class="gl-head"><?php
		_e( 'Footer Widget Body', 'cuttz-framework' );
		?></h4></td>
			  </tr>
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'footer-widgets-font-family', $this->settings_field, false ) );
...
?>
		...
		?></label>
				...
				<td class="cuttz-input"><p>
				<?php
		$weights = cuttz_font_weights( genesis_get_option( 'footer-font-family', $this->settings_field, false ) );
...
/**
 * Registers the Cuttz Design Settings Admin Menu
 * @return none
 */
function cuttz_design_settings_menu() {
	global $_cuttz_admin_skin_settings;
	$_cuttz_admin_skin_settings = new Cuttz_Skin_Settings;
}